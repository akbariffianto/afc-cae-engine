# ARCHITECTURAL DESIGN SPECIFICATION (DESIGN.MD)
## SYSTEM: AFC-CAE EXECUTIVE DASHBOARD (UI LAYER) — DOCKER ENTERPRISE EDITION
**Document Version:** 1.2  
**Target Framework:** Streamlit (Python 3.10+)  
**Deployment Model:** Strategy 2 (Dockerized Cloud/VPS Containerization)  
**Classification:** INTERNAL — REVIEWS READY (PwC Portfolio Verification)

---

## 1. ARCHITECTURAL OVERVIEW (STRATEGY 2)
To achieve complete environmental isolation, high portability, and 24/7 availability for enterprise review, the AFC-CAE user interface is decoupled into a standalone, lightweight Docker container. 

This micro-frontend securely communicates across the network with the core FastAPI backend container via non-blocking asynchronous REST API polling requests, maximizing memory efficiency on standard host environments.

+----------------------------------------------------------------------------------------+
|                                  DOCKER CONTAINER A                                    |
|                             STREAMLIT DASHBOARD SERVICE                                |
|   (Sidebar Ingestion) ---> (Asynchronous Polling Loop) ---> (Dynamic Rendering Engines)  |
+----------------------------------------------------------------------------------------+
|
| HTTP REST API Requests (JSON Across Network)
v
+----------------------------------------------------------------------------------------+
|                                  DOCKER CONTAINER B                                    |
|                                FASTAPI AUDIT ENGINE SERVICE                            |
|     [/api/v1/audit/ingest] (Acceptance)  <--->  [/api/v1/audit/status/{run_id}]        |
+----------------------------------------------------------------------------------------+


---

## 2. THE UI DESIGN SYSTEM & PALETTE
The dashboard layout follows a strict, professional corporate taxonomy to suit the preferences of data governance leads, executive managers, and financial consultants:

* **Primary Identity Color:** `#D04A02` (Muted Terracotta Orange)
* **Canvas Background:** `#F8F9FA` (Clean Soft Grey)
* **Surface Cards:** `#FFFFFF` (Pure White Containers for KPI Data)
* **Typography Accent:** `#1C1C1C` (Deep Charcoal for optimal scanning)
* **Dynamic Alert Triggers:**
    * `Skenario A (Safe/Operational Variance):` `#2E7D32` (Desaturated Forest Green)
    * `Skenario B (Watch Zone / Localized Anomalies):` `#EF6C00` (Amber Ochre Orange)
    * `Skenario C (Critical Systemic Halt):` `#C62828` (Crimson Wine Red)

---

## 3. CORE FRONTEND STREAMLIT SCRIPT (`src/frontend/app.py`)

OpenCode must write this complete, end-to-end Python script directly into `src/frontend/app.py`:
...
# Page setup for enterprise styling
st.set_page_config(
...
# Custom CSS injection for clean grid layouts and desaturated dashboard cards
st.markdown(\"\"\"
    <style>
...
# Containerized URL Configuration (Read from environment variables, fallback to local localhost)
BACKEND_HOST = os.getenv("FASTAPI_BACKEND_URL", "[http://127.0.0.1:8000](http://127.0.0.1:8000)")
INGEST_ENDPOINT = f"{BACKEND_HOST}/api/v1/audit/ingest"
STATUS_ENDPOINT_TEMPLATE = f"{BACKEND_HOST}/api/v1/audit/status/{{run_id}}"

# Initialize cross-thread session states for stability
...
# --- SIDEBAR LAYER ---
st.sidebar.image("[https://img.icons8.com/external-flatart-icons-flat-flatarticons/128/external-audit-business-and-finance-flatart-icons-flat-flatarticons.png](https://img.icons8.com/external-flatart-icons-flat-flatarticons/128/external-audit-business-and-finance-flatart-icons-flat-flatarticons.png)", width=65)
...
if st.session_state.audit_data:
    ...
    if scenario == "A":
        ...
        bg_color, badge_text = "⚠️ SKENARIO C: EMERGENCY HALT | Kontaminasi Sistemik Masif (> 5%)"
    ...
    st.info("👋 System Idle. Please load a transaction ledger subset in the control center to start automated a